...
fn run() -> Result<()> {
    ...
    for full in &remotes_full {
        ...
        // --format=%(refname:short)` will sometimes emit a bare remote name
        // (e.g. `origin`) for `refs/remotes/<remote>/HEAD` — skip those, they
        // aren't checkout targets.
    ...
    match chosen.kind {
        // `git switch` handles local branches and DWIM-tracks remote-only.
